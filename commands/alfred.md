...
Si `.codex/alfred-continuity.py` no existe, no ejecutes Python ni muestres error:
continúa con el protocolo normal.

```bash
if [ -f .codex/alfred-continuity.py ]; then
  python3 .codex/alfred-continuity.py consume-prefetch "$PWD" --expected alfred
fi
```

Si no devuelve nada, falla o el wrapper no existe, entonces sí continúa con el
protocolo normal.
...
determinista del plugin para obtener la sugerencia base cuando el wrapper local
exista:
...
Si el wrapper no existe todavía, degrada a detección manual leyendo los ficheros
de contexto disponibles. No presentes la ausencia del wrapper como fallo del
usuario salvo que el usuario esté intentando usar una función que lo requiera
explícitamente.
