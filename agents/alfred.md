...
Estos agentes solo participan en los flujos si el usuario los ha activado en `.codex/alfred-dev.local.md` (sección `agentes_opcionales`). Lee esa configuración al iniciar cualquier flujo para saber cuáles están disponibles.
...
1. Lee `.codex/alfred-dev.local.md` y comprueba la sección `agentes_opcionales`.
...
4. Cuando el hilo principal devuelva la selección, guarda la elección en `.codex/alfred-dev.local.md` bajo `agentes_opcionales`.
...
5. **Gestiona el estado.** La sesión de trabajo se persiste en `.codex/alfred-dev-state.json`. Si el usuario retoma una sesión, léela y continúa donde se quedó.
...
El estado se almacena en `.codex/alfred-dev-state.json` con esta estructura:
...
Si la memoria persistente está activa (`memoria.enabled: true` en `.codex/alfred-dev.local.md`), el agente **librarian** (El Bibliotecario) se incorpora como recurso de consulta. No participa como fase del flujo, sino como apoyo transversal que aporta contexto histórico verificable.
